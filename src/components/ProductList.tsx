...
export function ProductList({ products }: { products: ProductListItem[] }) {
  if (products.length === 0) {
    return <div className="rounded-3xl border border-dashed border-slate-200 bg-white p-10 text-center text-slate-500 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">No products available yet.</div>;
  }
        ...
        <article key={product.id} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:border-slate-800 dark:bg-slate-950">
          <div className="grid gap-0 md:grid-cols-[280px_1fr]">
            <div className="relative min-h-64 overflow-hidden bg-slate-100">
              <MarketplaceImage src={product.image} alt={product.title} fill className="object-cover" sizes="(max-width: 768px) 100vw, 280px" />
            </div>
            <div className="p-6">
              <div className="flex flex-wrap items-center gap-3">
                <span className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500 dark:text-slate-400">{product.subtitle}</span>
                <Badge>{product.availability}</Badge>
              </div>
              <Link href={`/products/${product.slug}`} className="mt-4 block text-2xl font-semibold text-slate-950 transition hover:text-slate-700 dark:text-white dark:hover:text-slate-200">
                {product.title}
              </Link>
              <p className="mt-4 max-w-xl text-sm leading-6 text-slate-600 dark:text-slate-400">{product.description}</p>
              <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="text-2xl font-semibold text-slate-950 dark:text-white">{formatINR(product.price)}</p>
