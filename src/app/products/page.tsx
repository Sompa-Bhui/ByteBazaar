...
export default async function ProductsPage({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  const params = new URLSearchParams(normalizedSearchParams as Record<string, string>);
      ...
      <div className="grid gap-10 xl:grid-cols-[280px_1fr]">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-3 space-y-2">
                <Button as="a" href={buildQuery(params, 'category', undefined)} variant={!searchParams.category ? 'primary' : 'secondary'} size="sm">
                  All categories
                </Button>
                {filters.categories.map((category) => (
                  <Button
                    key={category.value}
                    as="a"
                    href={buildQuery(params, 'category', category.value)}
                    variant={searchParams.category === category.value ? 'primary' : 'secondary'}
                    size="sm"
                  >
                    {category.label}
                  </Button>
            ...
            <div>
              ...
              <div className="mt-3 space-y-2">
                <Button as="a" href={buildQuery(params, 'brand', undefined)} variant={!searchParams.brand ? 'primary' : 'secondary'} size="sm">
                  All brands
                </Button>
                {filters.brands.map((brand) => (
                  <Button
                    key={brand.value}
                    as="a"
                    href={buildQuery(params, 'brand', brand.value)}
                    variant={searchParams.brand === brand.value ? 'primary' : 'secondary'}
                    size="sm"
                  >
                    {brand.label}
                  </Button>
              ...
              <div className="mt-3 space-y-2">
                {priceRanges.map((range) => (
                  <Button
                    key={range.value}
                    as="a"
                    href={buildQuery(params, 'price', range.value)}
                    variant={searchParams.price === range.value ? 'primary' : 'secondary'}
                    size="sm"
                  >
                    {range.label}
                  </Button>
              ...
              <div className="mt-3 space-y-2">
                {sortOptions.map((option) => (
                  <Button
                    key={option.value}
                    as="a"
                    href={buildQuery(params, 'sort', option.value)}
                    variant={searchParams.sort === option.value ? 'primary' : 'secondary'}
                    size="sm"
                  >
                    {option.label}
                  </Button>
        ...
        <section className="space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
            <div className="text-sm text-slate-500">Page {result.page} of {result.totalPages}</div>
            <div className="flex flex-wrap items-center gap-2">
              <Button as="a" href={buildQuery(params, 'page', Math.max(1, result.page - 1).toString())} variant="secondary" size="sm" disabled={result.page <= 1}>
                Previous
              </Button>
              <Button as="a" href={buildQuery(params, 'page', Math.min(result.totalPages, result.page + 1).toString())} variant="secondary" size="sm" disabled={result.page >= result.totalPages}>
                Next
              </Button>
            </div>
          </div>
