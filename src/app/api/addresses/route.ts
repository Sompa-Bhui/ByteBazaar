...
export async function GET(req: Request) {
  ...
  const addresses = await prisma.address.findMany({ where: { userId: dbUser?.id ?? '' }, orderBy: [{ isDefault: 'desc' }, { createdAt: 'desc' }] });
...
export async function POST(req: Request) {
  ...
  const created = await prisma.$transaction(async (tx) => {
    ...
    if (body.isDefault) await setDefaultAddress(tx, dbUser.id, address.id);
    return address;
  });
