...
export default function CreateBrand() {
  ...
  async function save(e: React.FormEvent) {
    e.preventDefault();
    const res = await fetch('/api/admin/brands', { method: 'POST', body: JSON.stringify({ name, slug, website }), headers: { 'Content-Type': 'application/json' } });
  ...
  return (
    <form onSubmit={save} className="space-y-4">
      ...
      <div>
        ...
        <input value={website} onChange={(e) => setWebsite(e.target.value)} className="w-full border p-2 rounded" />
      </div>
      <div>
